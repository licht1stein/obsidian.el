...
Obsidian.el provides an Emacs front-end for [[https://obsidian.md/][Obsidian Notes]].  Obsidian does not need to be installed to make use of the full functionality of this package.
...
** Following backlinks
If ~obsidian-backlinks-mode~ is enabled, running the command ~obsidian-backlink-jump~ will move the point to and from the backlinks panel.  From the backlinks panel, a backlink can be visited using ~obsidian-follow-link-at-point~.
...
If backlinks mode is active, a call to ~obsidian-backlink-jump~ will move the point to the backlinks panel where a link can be visited using ~obsidian-follow-link-at-point~.  Alternatively, a call to ~obsidian-backlink-jump~ from within the backlinks panel will return the point to the previously visited note window.
...
If ~obsidian-backlinks-mode~ is active when ~obsidian-backlink-jump~ is called, the point will move to the backlinks panel.  From the backlinks panel, notes can be visited using ~obsidian-follow-link-at-point~.
...
You can also simply type in a tag and obsidian.el and it will be recognized by obsidian.el.
